<?php
		...
		$foods = pdo_fetch("SELECT id FROM ".tablename('jufeng_wcy_foods')." WHERE id = :id", array(':id' => $foodsid));
		if (empty($foods)) {
		...
		$pcatetotal += $pcatecart['total'];
			...
			if($pricetotal < $category1[0]['sendprice']){
				$a =$category1[0]['sendprice']-$pricetotal;
